...
try:
    ...
    import re,tokenize
    tokenprog = re.compile(tokenize.Token)
...
def reparam2(string_, dictionary):
...
def sqlify(obj): 
    ...
    if obj is None:
        ...
        try:#modify by libyyu
            if sys.version_info >= (3, 0): 
                return '\'' + str(obj)+'\''
            else:
                from libs.convert import encoding
                js = encoding.to_utf8(obj)
                return '\''+js+'\''
        except:
...
class DB: 
    ...
    def __init__(self, db_module, keywords):
        ...
        self.max_idle_time = self.keywords.pop('max_idle_time', 7 * 3600)
    ...
    def _getctx(self): 
        ...
        elif time.time() - self._last_use_time > self.max_idle_time:
            self._load_context(self._ctx)
        self._last_use_time = time.time()
        return self._ctx
    ...
    def has_table(self, table):
        ...
        if isinstance(out, int): 
            return out > 0
        elif isinstance(out, iterbetter):
            out.first()
            return not not out
...
class SqliteDB(DB): 
    def __init__(self, **keywords):
        ...
        keys = list(keywords.keys()) if sys.version_info > (3,0) else keywords.keys()
        for k in keys:
            if k not in valid_param:
                del keywords[k]
    ...
    def has_table(self, table):
        out = self.query("select count(*)  from sqlite_master where type='table' and name='%s'" % table)
        if not out: return False
        first = out.first()
        return not not first and first['count(*)'] == 1
...
class SafeEval(object):
    ...
    def eval_node(self, node, mapping):
        if node.type == "text":
            ...
            t = sqlquote(self.eval_expr(node, mapping))
            return t
