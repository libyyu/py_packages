...
def safeunicode(obj, encoding='utf-8'):
    ...
    """
    t = type(obj)
    if t is (str if sys.version_info >= (3,0) else unicode):
        return obj
    elif t is str:
        return obj.decode(encoding)
    elif t in [int, float, bool]:
        return unicode(obj) if sys.version_info < (3,0) else str(obj)
    elif hasattr(obj, '__unicode__') or isinstance(obj, str if sys.version_info >= (3,0) else unicode):
        return unicode(obj)
    else:
        return str(obj).decode(encoding) if sys.version_info < (3,0) else str(obj)
...
class IterBetter:
    ...
    def first(self, default=None):
        ...
        """
        try:
            return iter(self).next() if sys.version_info < (3, 0) else next(iter(self))
    ...
    def __getitem__(self, i):
        ...
        try:
            while i > self.c:
                if sys.version_info < (3, 0): self.i.next()
                else: next(self.i)
                self.c += 1
            # now self.c == i
            self.c += 1
            return self.i.next() if sys.version_info < (3, 0) else next(self.i)
        except StopIteration: 
    ...
    def __nonzero__(self):
        if hasattr(self, "__len__"):
            ...
            try:
                self._head = self.i.next() if sys.version_info < (3, 0) else next(self.i)
            except StopIteration:
                return False
            else:
                return True
...
def safeiter(it, cleanup=None, ignore_errors=True):
    ...
    def next():
        while True:
            try:
                return it.next() if sys.version_info < (3, 0) else next(it)
            except StopIteration:
                raise
            except:
                traceback.print_exc()
...
def safewrite(filename, content, encoding='UTF-8'):
    ...
    if sys.version_info < (3,0):
        ...
        f = __builtin__.open(filename + '.tmp', mode='w', encoding=encoding)
    f.write(content)
    f.close()
